...
class Battle extends FlameGame {
  ...
  Battle({required this.onTurnOver});
  ...
  void changeSprite(int player, int action) async {
    ...
    // var spriteSheetMushWalk = await images.load('MushWalk.png');
  ...
  Future<void> onLoad() async {
    ...
    SpriteAnimationData spriteSkeletonIdle = SpriteAnimationData.sequenced(
        amount: 4, stepTime: 0.1, textureSize: spriteSize);

    SpriteAnimationData spriteData2 = SpriteAnimationData.sequenced(
        amount: 4, stepTime: 0.1, textureSize: spriteSize);
    // Carga el sprite del corazón

    // Añade tres corazones a la lista

    background
      ..sprite = await loadSprite('BackgroundDia.png')
      ..size = canvasSize;
    player1 = SpriteAnimationComponent.fromFrameData(
        spriteSheetSkeletonIdle, spriteSkeletonIdle)
      ..x = 25
      ..y = screenHeight / 4;
    player2 = SpriteAnimationComponent.fromFrameData(
        spriteSheetGoblinIdle, spriteData2)
      ..x = screenwidth - 20
      ..y = screenHeight / 4;
    player2.flipHorizontally();
